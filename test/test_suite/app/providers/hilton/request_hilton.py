...
class RequesFailedHiltonRequest(Exception):
    pass


def hilton_request(params: RequestParameters, request_id: int) -> list[RunRequest]:
    """Execute a single request with retries - designed to be run in a thread"""
    url = params["url"]
    if params.get("request_proxy") is None:
        raise RequesFailedHiltonRequest("No proxies")
    request_proxy_str: str = str(params.get("request_proxy", ""))
    ...
    for i in range(request_maximum_retries):
        ...
        try:
            ...
            if not is_successful:
                raise RequesFailedHiltonRequest(
